...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = pac
    variableDomainsDict[GHOST0] = g_zero
    variableDomainsDict[GHOST1] = g_one
    obs = X_RANGE + Y_RANGE + MAX_NOISE
    variableDomainsDict[OBS0] = range(obs - 1)
    variableDomainsDict[OBS1] = range(obs - 1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        curr = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for e in eliminationOrder:
            curr, joined = joinFactorsByVariable(curr, e)

            uncond_joined = joined.unconditionedVariables()
            if len(uncond_joined) != 1:
                curr.append(eliminate(joined, e))

        return normalize(joinFactors(curr))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None and jailPosition == ghostPosition:
            return 1
        if noisyDistance == None or jailPosition == ghostPosition:
            return 0

        d = manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(noisyDistance, d)
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for g in self.allPositions:
            ...
            ev = self.getObservationProb(noisyDistance, pac_pos, ghost_pos, jail_pos)
            self.beliefs[g] = self.beliefs[g] * ev
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old in self.allPositions:
            trns = self.getPositionDistribution(gameState, old)
            oldp = self.beliefs[old]
            for p in self.allPositions:
                pro = trns[p] * oldp
                belief[p] = belief[p] + pro
        self.beliefs = belief
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            belief[p] = belief[p] + (1 / self.numParticles)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for g in self.particles:
            dist[g] = dist[g] + self.getObservationProb(observation, gameState.getPacmanPosition(), g, self.getJailPosition())
    ...
    def elapseTime(self, gameState):
        ...
        for old in self.particles:
            loc.append(self.getPositionDistribution(gameState, old).sample())
        self.particles = loc
        "*** END YOUR CODE HERE ***"
